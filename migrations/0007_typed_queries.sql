...
-- answer_space now defaults to the text space (set by the ADD COLUMN above),
-- so it no longer distinguishes legacy rows from anything else. subject still
-- defaults to '{}', so it is the marker for "not yet touched by this backfill".
